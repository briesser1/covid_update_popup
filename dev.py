...
df = pd.concat([new_row, df[:]]).reset_index(drop = True)
	...
	#remove old dates and data from df_storage.csv
df_cleaner = pd.read_csv('C:/Users/Riesser/Documents/python_popup/df_storage.csv')
df_cleaner = df_cleaner.drop('Unnamed: 0', 1)

	# convert data to datetime
df_cleaner['recorded_date'] = pd.to_datetime(df_cleaner['recorded_date'], format='%Y-%m-%d')
old_data_date = pd.to_datetime(old_data_date, format='%Y-%m-%d')


df_cleaner = df_cleaner[df_cleaner['recorded_date'] > old_data_date]
df_cleaner.to_csv('C:/Users/Riesser/Documents/python_popup/df_storage.csv')
